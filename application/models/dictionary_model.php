        ...
        if(!empty($_add)){
            ...
            if($dict_id == 7){
                ...
                foreach ($options_add as $val)
                {
                    $sorts = array_search($val,$options);
//                    $_add[] = array('parent_id'=>$dict_id,'name'=>$val,'sorts'=>$sorts);
                    $sql = "select id from est_dictionary_detail where name='".$val."' and parent_id=".$dict_id." and sorts=".$sorts;
                    $connect_result = $this->db_read->query($sql);
                    if ($connect_result->num_rows() > 0) {
                        $content_id = $connect_result->row()->id;
                    }
//                    foreach($lxjg->result_array() as $key => $val)
//                    {
//
//                    }
	...
	//新建信息来源
	public function add_option($data_from)
	{
		$this->db_read->select_max('sorts');
		$query = $this->db_read->get_where('est_dictionary_detail',array("parent_id"=>1));
		$max_sort = $query->row()->sorts;
		$max_sort = $max_sort+1;
		$add = array('parent_id'=>1,'name'=>$data_from,'sorts'=>$max_sort);
		$this->db_write->insert('est_dictionary_detail', $add);
	}
}