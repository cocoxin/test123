...
class User_model extends CI_Model
{
    ...
    private function get_user_codition($condition = array())
    {
        ...
        if (!empty($condition['role_id'])) {
            $wheres[] = "role_id =" . $condition['role_id'];
        }
        //部门（包括子部门）
        ...
        if (!empty($condition['except_userids'])) {
            $wheres[] = "user_id NOT IN(" . $condition['except_userids'] . ")";
        }
        /*全局查询，不设置数据权限*/
    ...
    public function get_free_users()
    {
        ...
        if ($code != 200 || empty($data)) {
            ...
            $responce->rows  = array();
            return $responce;
        } else if (count($data) == 1 && $data[0]['ag_id'] == $user_session_id) {
            //判断是否只有当前自己一个坐席
            $responce->total = 0;
            $responce->rows  = array();
            return $responce;
    ...
    public function update_user($data, $where)
    {
        ...
        $user_field   = array('user_name', 'dept_id', 'dept_name', 'user_remark', 'user_tel_server', 'user_cti_server', 'user_channel', 'user_sms_phone', 'user_if_tip', 'user_ol_model', 'user_outcaller_type',
            'user_outcaller_num', 'user_login_state', 'wechat_auth', 'user_phone_type', 'role_id', 'role_name', 'role_type', 'user_display_dialpanel', 'user_outcall_popup', 'user_to_selfphone', 'wechat_nickname', 'wechat_welcome_flag', 'wechat_welcome_message', 'switch_welcome_flag', 'switch_welcome_message', 'wechat_end_flag', 'wechat_end_message');
        ...
        //2、后台设置转手机功能
        if (isset($update_array['user_to_selfphone']) && $where['user_id'] && !empty($update_array['user_sms_phone'])) {
            ...
            $phone_id = $this->set_pstnPhone($update_array['user_sms_phone']);
            $islog = 1;
            if($state==1){
                if($islog)
                {
                    $this->agent_logout($where['user_id'], 1);
                    $res_login = $this->agent_login($phone_id,$where['user_id'],$update_array['user_sms_phone']);
                    return TRUE;
                }
            }else{
                $res_logout = $this->agent_logout($where['user_id']);
                return TRUE;
            }
    ...
    public function insert_user($insert = array())
    {
        ...
        $user_fields = array('user_name', 'user_num', 'role_id', 'dept_id', 'dept_name', 'user_remark', 'user_ol_model', 'user_outcaller_type', 'user_outcaller_num', 'user_password', 'user_login_state', 'wechat_auth',
    ...
    /**
     * 部门及部门下人 树 (异步获取数据)
     * 待分配：部门-姓名-工号-角色-当前任务
     * 已分配：部门-姓名-工号-角色
     * @param int $dept_id
     * @return array
     */
    public function get_task_user_tree($dept_id = 0)
    {
        $dept_session_id = $this->session->userdata('dept_id');
        $this->load->model('department_model');
        $dept_tree_arr = array();
    ...
    /**
     * 通过user_id取得$user_name     * @param string $user_name 员工姓名
     * @return int user_id
     * @author zgx
     */
    public function get_user_name_by_user_id($user_id = '')
    {
        ...
        $user_query = $this->db_read->get_where('est_users', $where);
        if ($user_query->num_rows() > 0) {
            return $user_query->row()->user_name;
        } else {
            return '';
        }
    }
     ...
     * @return string
     */
    public function get_dept_name_by_user_id($user_id = '')
    {
        $where      = array('user_id' => $user_id);
        $user_query = $this->db_read->get_where('est_users', $where, 1);
        if ($user_query->num_rows() > 0) {
            return $user_query->row()->dept_name;
        } else {
            return '';
        }
    }
    ...
    /**
     * 根据user_id获取user_num,user_name
     */
    public function get_num_name_by_id($user_id)
    {
        $this->db_read->select('user_num,user_name');
        $this->db_read->where('user_id', $user_id);
        $query = $this->db_read->get('est_users');
        return $query->row_array();
    }

    public function getQcUser($condition)
    {
        $wheres = $this->get_user_codition($condition);
        // if (!empty($condition['qc_id'])) {
        //     $this->load->model('quality_check_model');
        //     $id_arr = $this->quality_check_model->getProjectUserId($condition['qc_id']);
        //     if ($id_arr) {
        //         $wheres[] = "user_id IN (".implode(',',$id_arr).")";
        //     } else {
        ...
        //     if ($id_arr)
        //     {
        //         $wheres[] = "user_id NOT IN (".implode(',',$id_arr).")";
        //     }
        // }
        $nopage      = isset($condition['nopage']) ? $condition['nopage'] : 0;
        $select_flag = !isset($condition['is_selected']) ? 1 : $condition['is_selected'];
        $qc_id       = !isset($condition['qc_id']) ? 0 : $condition['qc_id'];
        if ($qc_id) {
            $this->load->model('quality_check_model');
            $id_arr = $this->quality_check_model->getProjectUserId($condition['qc_id']);
            if ($id_arr) {
        ...
        $this->db_read->select('user_num,est_users.user_id,role_name,dept_name,user_name');
        $this->db_read->from('est_users');
        $user_rows = $this->db_read->get();
        // $this->db_read->where()
        $user_array = $user_rows->result_array();
        $user_rows->free_result();
        $this->load->model('quality_check_model');
        foreach ($user_array as $key => $value) {
            $value['user']     = $value['user_num'] . " " . $value['user_name'];
            $value['pro_name'] = $this->quality_check_model->getUserQcProjectName($value['user_id']);
            $response->rows[]  = $value;
        }
        return $response;
    }
     ...
     *获取所有坐席
     */
    public function get_wkf_user_box()
    {
        if (!empty($where)) {
            $this->db_read->where($where);
        }
        ...
        if (!empty($result)) {
            foreach ($result as $value) {
                $user_box[] = array('user_id' => $value["user_id"], 'user_name_num' => $value["user_name"] . "[" . $value["user_num"] . "]", 'user_num' => $value['user_num']);
            }
        }
        return $user_box;
    }
     ...
     * @param $phone_num
     */
    public function agent_login($phone_id, $ag_id, $phone_num)
    {
        $win_db = $this->load->database('wintels', true);
        $sql = "SELECT a.vcc_id,a.ag_num,a.ag_name,getagqu(a.id) agqu, b.pho_num pho_num, b.pho_type,b.pho_chan
                    FROM win_agent a left join win_phone b on a.vcc_id=b.vcc_id and b.id='$phone_id' WHERE a.is_del=0 and a.id='$ag_id'";
        $result = $win_db->query($sql);
        ...
        $str = "aglogin(".$ag_id."|".$info[0]['vcc_id']."|".$phone_id."|".$info[0]['ag_num']."|".$ag_name."|".$info[0]['pho_num']."|".$info[0]['pho_type']."|".$info[0]['agqu']."|".$info[0]['pho_chan'].")";
        $address  = $this->config->item('default_tel_server');//通信服务器ip
        $port = "5015";
        ...
        {
            return array("code"=>200, "message"=>"静态登录成功");
//                make_json_result($phone_num."静态登录成功");
        }
        else
        {
            $responce = explode("\r\n",$responce);
            ...
            {
                case '1': $errname = "坐席已初始化";break;
                case '2': $errname = "本Socket已初始化";break;
                case '3': $errname = "分机号不存在";break;
                case '4': $errname = "分机已有坐席使用";break;
                case '5': $errname = "有队列不存在";break;
                case '6': $errname = "已达登陆上限";break;
                case '7': $errname = "分机类型错误";break;
                default: $errname = "格式错误";break;
            }
            make_json_error($errname);
        }
    }
     ...
     * @return array|string
     */
    public function agent_logout($ag_id, $flag = 0)
    {
        $str = "aglogout(".$ag_id.")";
        $address  = $this->config->item('default_tel_server');//通信服务器ip
        $port = "5015";
        ...
        if(strstr($responce,'Success'))
        {
            //make_json_result(1,"静态退出成功");
            if(empty($flag))
            {
                return array("code"=>200, "message"=>"静态退出成功");
            }
        }
        else
        {
            ...
            switch(trim($errcode[1]))
            {
                case '1': $errname = "坐席不存在";break;
                case '2': $errname = "不是静态坐席";break;
                case '3': $errname = "坐席已从页面登录";break;
                default: $errname = "格式错误";break;
            }
            if($errname == "坐席不存在")
            {
                return array("code"=>200, "message"=>"您未登录");
            }else{
                make_json_error($errname);
            }
        }
     ...
     * @return mixed
     */
    public function set_pstnPhone($phone_num)
    {
        $win_db = $this->load->database('wintels', true);
        $phone_id = 0;
        $select_query = $win_db->query("SELECT id FROM win_phone WHERE pho_num = '{$phone_num}' AND pho_type='1' LIMIT 1");

        if($select_query->num_rows()){
            $phone_id = $select_query->row()->id;
        }else{
            $data = array(
                'vcc_id'=>$this->session->userdata('vcc_id'),
                'pho_num'=>$phone_num,
            ...
            $win_db->insert('win_phone',$data);
            $phone_id = $win_db->insert_id();
        }
        return $phone_id;
    }
     ...
     * @param $ag_id
     */
    public function agent_exit($ag_id)
    {
        $msg = "agexit($ag_id)";
        $win_db = $this->load->database('wintels', true);
        $sql = "SELECT id,ag_type FROM win_agent WHERE is_del = 0 AND id = '" . $ag_id ."'";
        $query  = $win_db->query($sql);
        $result = $query->row_array();
        if(empty($result)){
            make_json_error('您要操作的坐席不存在！');
        } else {
            if($result['ag_type'] == 2)
            {
                make_json_error('坐席是静态坐席，不能强制操作！');
            }
            else
            {
                $res = agent_operate($msg);
                $errname = "坐席不存在";
                if($res === true || $res == $errname)
                {
//                    make_json_result();
                }
                elseif($res === false)
                {
                    make_json_error('连接通信服务器失败');
                }
                else
                {
                    make_json_error($res);
