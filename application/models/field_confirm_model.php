...
class Field_confirm_model extends CI_Model {
	...
	private function delete_columns($columns_array)
	{
		foreach ($columns_array as  $columns) {
			$this->db_write->where('if_custom', 1);
			$this->db_write->where('field_type', 0);
			$this->db_write->where('fields', $columns);
			$this->db_write->update('est_fields', array('is_delete' => 1));
		}
		return true;
	}
	...
	private function operation_database_column($fields,$table_name,$opt_type)
	{
		...
		switch($opt_type)
		{
			case 'add':
				...
				{
					$data_field[$field] = "ADD $field VARCHAR(50) DEFAULT '' NOT NULL";
				}
				$data_field_str = implode(",",$data_field);
				break;
			case 'del':
			...
			case 'update_to_text':
				foreach ($fields AS $field)
				{
					$data_field[$field] = "MODIFY COLUMN $field text";
				}
				$data_field_str = implode(",",$data_field);
				break;
			case 'update_to_jl':
				foreach($fields AS $field)
				{
					$data_field[$field."_1"] = "ADD ".$field."_1 text";
					$data_field[$field."_2"] = "ADD ".$field."_2 text";
					$data_field[$field."_3"] = "ADD ".$field."_3 text";
				}
				$data_field_str = implode(",",$data_field);
				break;
			default:
		...
		$str_sql = "ALTER TABLE ".$table_name." ".$data_field_str;

		//导入客户数据
		if ($table_name == 'est_client')
		{
			$this->db_write->query("ALTER TABLE est_import_error_data ".$data_field_str);//错误数据表
			if($this->db_read->table_exists('est_client_history'))
			{
				$this->db_write->query("ALTER TABLE est_client_history ".$data_field_str);//客户历史数据表
			}
			//当有表更新时更新项目表
			if($this->db_read->table_exists('est_project_client'))
			{
				/*
				 * 项目表更改处理
				 */
				...
				if( $opt_type != 'del')
				{
		            ...
		            $enable = $this->task_import_model->getProClientFields(FALSE); 
		            $enable_keys  = array_keys($enable) ; 
		            //查找在表中已经存在的字段 取交集
		            $diff = array_intersect($enable_keys, $fields);

		            foreach ($diff as $field_key) {
		            	if(  isset( $data_field[$field_key] ) ) 
						{
							//已经存在的field清除掉
							unset($data_field[$field_key]);
						}
		            }
		            $data_field_str = implode(",",$data_field);
					$this->db_write->query("ALTER TABLE est_project_client ".$data_field_str);//
				}
			}
		}
	...
	public function del_two_fields($field_type,$first_unit='',$second_unit='')
	{
		...
		if ($field_type ==FIELD_TYPE_CLIENT) {
			$this->delete_columns(array($field_info1['fields'], $field_info2['fields']));
		} else {
			//删除字段选项表信息
