<?php /* Smarty version 2.6.19, created on 2016-11-15 10:51:46
         compiled from field_confirm_option.htm */ ?>
...
"/>
 ...
 <tr>
      <td>
        2、<input type="checkbox" name="field_require" value="" <?php if ($this->_tpl_vars['field_info']['if_require'] == 2): ?>checked<?php endif; ?>> 是否必填项
      </td>
 </tr>
 <tr>
     <td>
...
" name="field_data_type" style="margin-bottom:0px;" onchange="change_date_type()">
 ...
 <tr class="datefmt" style="display:none;">
  <td>
       日期格式
       &nbsp;&nbsp;&nbsp;<input type="radio" name="datefmt" id="dateFmt1" value="yyyy-MM-dd" <?php if ($this->_tpl_vars['field_info']['datefmt'] != 'yyyy-MM-dd HH:mm:ss'): ?>checked<?php endif; ?> > 年-月-日
       &nbsp;&nbsp;&nbsp;<input type="radio" name="datefmt" id="dateFmt2" value="yyyy-MM-dd HH:mm:ss" <?php if ($this->_tpl_vars['field_info']['datefmt'] == 'yyyy-MM-dd HH:mm:ss'): ?>checked<?php endif; ?> > 年-月-日 时：分：秒
  </td>
 </tr>
...
">
 		 	选项设置 
 		 	<!-- <button id="select-btn-plus<?php echo $this->_tpl_vars['parent_id']; ?>
" type="button" class="btn btn-mini btn-primary" style="margin-bottom: 4px;"><span class="glyphicon glyphicon-plus"></span></button> -->
 		 	<!-- <button class="new_other_btn" type="button" id="select-btn-plus<?php echo $this->_tpl_vars['parent_id']; ?>
" style="margin-bottom: 4px;">
            <span class="iconfont">&#xe626;&nbsp;<span class="btn-size"></span></span></button>  -->
            <a class="iconfont" id="select-btn-plus<?php echo $this->_tpl_vars['parent_id']; ?>
">&nbsp;&nbsp;<span class="btn-size">添加</span></a>
...
">
             		<button type="button" name="btn-minus" class="btn btn-mini btn-danger" style="margin: 0px 0px 2px -3px;">
			...
			<?php else: ?>
 				<div class="controls">
             		...
             		<button type="button" name="btn-minus" class="btn btn-mini btn-danger red" style="margin: 0px 0px 2px -3px;">
                 		<span class="glyphicon glyphicon-minus"></span>
             		</button>
        		</div>
        		<div class="controls">
...
<tr class="more_select_jl_tr" style="display:none;">
 	...
 	<form class="form-inline">
      ...
      <button type="button" name="select_jl-1-btn" class="btn btn-small btn-primary" onclick="set_select_jl_option(1)" style="margin-left:-3px;">设置</button>
      <select name="select_jl_2" style="width:90px;">
      	<option value="">--第2级--</option>
      </select>
      <button type="button" name="select_jl-2-btn" class="btn btn-small btn-primary" onclick="set_select_jl_option(2)" style="margin-left:-3px;">设置</button>
    </form>
    </td>
 </tr>
<!--关联多选设置 end -->
...
'>
      ...
      <button type="button" name="jl-1-btn" class="btn btn-small btn-primary" onclick="set_jl_1()" style="margin-left:-3px;">设置</button>
      <select name="jl_2" style="width:90px;">
      	<option value="">--第2级--</option>
      </select>
      <button type="button" name="jl-2-btn" class="btn btn-small btn-primary" onclick="set_jl_2()" style="margin-left:-3px;">设置</button>
    </form>
    </td>
 </tr>
...
" />
			<button class="new_con_btn" type="button" id="_option_save<?php echo $this->_tpl_vars['parent_id']; ?>
" onclick="_save_submit('<?php echo $this->_tpl_vars['field_info']['field_type']; ?>
',<?php echo $this->_tpl_vars['parent_id']; ?>
)" title="保存设置">
			    <span class="iconfont">&#xe637;&nbsp;<span class="btn-size">保存设置</span></span> 
			</button>
		</div>
 	</td>
 </tr>
...
').css('display','none');
    ...
    if("<?php echo $this->_tpl_vars['field_info']['default']; ?>
" != "" && "<?php echo $this->_tpl_vars['field_info']['default']; ?>
" != 'undefined'){
        $('input[name="field_default"]').val('<?php echo $this->_tpl_vars['field_info']['default']; ?>
');
    }
	if(select==2)
	{
...
').append('<div class="controls">\
             	...
             	<button type="button" name="jl-btn-minus" class="btn btn-mini btn-danger" style="margin: 0px 0px 2px -3px;">\
                 	<span class="glyphicon glyphicon-minus"></span>\
             	</button>\
        </div>');
...
').append('\
				<select name="jl_3" style="width:90px;">\
      			...
      			<button type="button" name="jl-3-btn" class="btn btn-small btn-primary" onclick="set_jl_3()" style="margin-left:-3px;">设置</button>');
	}
}
...
').parent().find('span').remove();
	$('#jl_option_div<?php echo $this->_tpl_vars['parent_id']; ?>
').append('第'+deep+'级选项设置\
		<a class="iconfont" onclick="add_jl_option()">&nbsp;&nbsp;<span class="btn-size">添加</span></a>\
		<a class="iconfont" onclick="save_jl_option(\'<?php echo $this->_tpl_vars['parent_id']; ?>
\','+deep+','+p_id+',\''+field_name+'\')">&nbsp;&nbsp;<span class="btn-size">保存</span></a>\
...
/*级联 - 保存选项*/
function save_jl_option(field_id,type,p_id,field_name)
{
	var _data = {};
	...
	$.ajax({
		...
		data:{"p_id":p_id,"options":_data,'type':type,'field_id':field_id},
		dataType:"json",
