...
class User extends CI_Controller {
	...
	public function self_update_user()
	{
		...
		$data = array(
            ...
            'wechat_welcome_flag'=>$this->input->post('wechat_welcome_flag'),
            'wechat_welcome_message'=>$this->input->post('wechat_welcome_message'),
            'switch_welcome_flag'=>$this->input->post('switch_welcome_flag'),
            'switch_welcome_message'=>$this->input->post('switch_welcome_message'),
            'wechat_end_flag'=>$this->input->post('wechat_end_flag'),
            'wechat_end_message'=>$this->input->post('wechat_end_message')
        );
	...
	public function output_user()
	{
		...
		$menu = array('姓名','工号','坐席电话','常接通模式','角色','部门','登录服务器IP','短信提醒号码','是否弹出提示窗口','外呼主叫号码类型','指定的外呼主叫','是否显示外呼窗口','是否外呼弹屏','最后登录IP','最后登录时间','备注');
		$data_info = array();
		$data_info[0] = $menu;

        $inarray = $_GET;//获取条件参数
		$this->load->model('user_model');
		$user_info = $this->user_model->output_user($inarray);

		for($i=1;$i<=count($user_info);$i++)
		{
			$data_info[$i] = $user_info[$i-1];
		

		$this->load->library("csv");a
		$this->csv->creatcsv('员工',$data_info);
	}
     ...
     *
     */
    public function get_task_user_tree()
    {
 
        $this->load->model("user_model");
        $dept_id = $this->input->post('id');
        $responce = $this->user_model->get_task_user_tree($dept_id);
        $this->load->library('json');
        echo $this->json->encode($responce);
    }
    ...
    public function get_all_user() 
    {
    	$this->load->model("user_model");
    	$condition = array();
    	$condition['gl_all_data'] = 1 ;
		$result = $this->user_model->get_user_box($condition,0);
		$this->load->library("json");
		echo $this->json->encode($result);
    }
}
