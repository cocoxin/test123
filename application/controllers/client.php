...
class Client extends CI_Controller {
	...
	public function insert_client()
	{
		...
		$this->load->model("client_model");
		$this->load->model("call_client_model");
		$user_id = $this->session->userdata('user_id');
		//判断是否超过客户限制
		...
		if($check_amount_result)
		{
			...
			$task_id = isset($inarray['task_id']) ? $inarray['task_id'] : null;
			$pro_id = isset($inarray['pro_id']) ? $inarray['pro_id'] : null;
			$call_id = isset($inarray['call_id']) ? $inarray['call_id'] : null;
			unset($inarray['task_id']);
			unset($inarray['pro_id']);
			unset($inarray['call_id']);
			...
			if ($result)
			{
				if ($task_id) {
					if ($call_id) {//更新转化客户id
						$result_data = array('task_id' => $task_id, 'call_id' => $call_id, 'pro_id' => $pro_id, 'is_transfer' => 1);
						$this->call_client_model->insert_project_result($result_data, array('call_id' => $call_id));
					}
					$up_result = $this->call_client_model->update_transfer_id($task_id, $result);
					if (!$up_result) {
						make_json_error("转化客户id更新失败！");
					}
				}
