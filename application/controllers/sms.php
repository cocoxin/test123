...
class Sms extends CI_Controller {
    ...
    public function sms_statistics()
    {
        ...
        $result=$this->user_model->get_user_box($condition);
        $this->smarty->assign("agent",$result);
        $this->smarty->display("sms_statistics.htm");
    }
     ...
     *
     */
    public function sms_statistics_query()
    {
        ...
        if (!empty($inarray["user_id"]))
        {//短信接收号码
            $wheres[] = "user_id = ".$inarray["user_id"];
        }
        if (!empty($inarray["send_start_time"]))
        {//发送时间-开始
            $wheres[] = "sms_send_time >= '".strtotime($inarray["send_start_time"])."'";
        ...
        list($page, $limit, $sort, $order) = get_list_param();
        $this->load->model("sms_model");
        $responce = $this->sms_model->get_sms_statistics($page, $limit, $sort, $order,$where);
        $this->load->library("json");
        echo $this->json->encode($responce);
    }
}