...
function remove_client()
{
	...
	if(ids == '')
	{
		$.messager.alert('错误','请选择要删除的客户数据','error');
		return;
	}
	var _relative_deal = '';
...
function delete_client(cle_id)
{
	$.ajax({
		...
		success: function(responce){
			...
			if(responce["error"] == 0)
			{
				$.messager.alert('提示',responce['content'],'info');
...
function release_client_by_id(cle_id)
{
	$.ajax({
		...
		success: function(responce){
			if(responce["error"] == 0)
			{
				$.messager.alert('提示(只能放弃属于自己的数据)',"已放弃"+responce['content']+"条数据",'info');
				$('#client_list').datagrid('load');
			}
			else
			{
...
function output_client(type)
{
    ...
    {
        $.messager.confirm('数据量大，建议用【导出CSV】导出数据', '确定继续导出excel吗？', function(r){
            if(r){
