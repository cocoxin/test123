...
(function($) {
	var wincall = function(opts) {
		$.extend(this, {
			...
			fn_dialouter: function(called,callerID,groupID,outChan){
                if(arguments[4])
                    var proID = arguments[4];
                else
                    var proID = 0;
                if(arguments[5])
                    var taskID = arguments[5];
                else
                    var taskID = 0;
            ...
            fn_transivr:function(){
                 return transivr();
             },
             fn_transque:function(transqueid){
             return transque(transqueid);
             },
			fn_get_que:function(){
				return get_queue();
			},
        ...
        var AGMT_TRANSIVR = 43      //转IVR
        var AGMT_TRANSQUE = 44      //转技能组
        ...
        var RETURN_INITSTATE= 6     //签入后状态消息
        var RETURN_IVRRESULT= 7     //IVR返回消息
		...
		function init(){
			...
			{
				$.getScript('./jssrc/jquery.activemq.js',function(){
		...
		function dialouter(called,callerID,groupID,outChan,proID,taskID)
		{
			...
			if(opts.dealnumber_before_call)
			{
				$.ajax({
					type:'POST',
					url: 'index.php?c=phone_control&m=dealnumber',
					data: {'number':called},
					dataType: 'json',
					success: function(responce){
						if(responce['error'] === 0)
						{
							called = responce['content'];
							called = called.replace(/[^0-9]/g,'');
        ...
        function transivr()
        {
            ...
            sendData({Action:AGMT_TRANSIVR,IvrID:0,IvrParm:"IVR"});
        }
        /* 转技能组*/
        function transque(transqueid)
        {
            if(!isLogin)
            {
                return false;
            }
            sendData({Action:AGMT_TRANSQUE,IvrID:0,TransType:1,TransParam1:transqueid});
        }
		...
		**/
		...
		function msgResponce(message){
			...
			else if(opts.connect_type == 'swf')
			{
                ...
                if (curentMessage.substr(0, 1) != '{' || curentMessage.substr(-1, 1) != '}') {
                    //curentMessage += message;
                    return;
                }

				var temp = curentMessage.split('\r\n');
                curentMessage = '';
			...
			switch(obj_responce.MT)
			{
				...
				switch(obj_responce.Retact)
				{
					...
					switch(obj_responce.Ret)
					{
						...
						case 810 : disconnect();return '001810';   //  查询参数[vcccode]为空
						case 811 : disconnect();return '001811';   //  查询参数[agnum]为空
						case 812 : disconnect();return '001812';   //  查询参数[agpass]为空
						case 813 : disconnect();return '001813';   //  查询参数[agphone]为空
						default:
                            disconnect();
                            var retCode = parseInt(obj_responce.Ret);
                            if (retCode >= 700 && retCode < 800) {
                                return '[001'+retCode+']坐席初始化接口访问失败';
                            }
                            return '001'+obj_responce.Ret;   //其它初始化错误
					}
					break;
					...
					switch(obj_responce.Ret)
					{
						...
						case 3 : return '009003';//项目错误(此项目非自动外呼项目或没有这个项目)
					...
					switch(obj_responce.Ret)
					{
						...
						case 3 : return '010003';//项目错误(此项目非自动外呼项目或没有这个项目)
					...
					switch(obj_responce.Ret)
					{
						...
						case 11 : return '022011';   //没有外呼权限(坐席没有外呼的权限)
					...
					switch(obj_responce.Ret)
					{
						...
						case 30 : return '030030';  //常接通模式不能监听
					...
					break;
                    case AGMT_TRANSIVR ://转IVR
                    switch(obj_responce.Ret)
                    {
                       case 0 : return '043000'; //转IVR成功
                       case 1 : return '043001'; //坐席错误
                       case 2 : return '043002'; //坐席不在通话中
                       case 3 : return '043003'; //坐席不在通话中
                       case 4 : return '043004'; //通话异常
                       case 5 : return '043005'; //通话类型错误
                       case 6 : return '043006'; //通话状态不对
                       case 7 : return '043007'; //通道错误
                       default: return '043999'; //其它转IVR错误
                    }
                    break;
                    case AGMT_TRANSQUE ://转技能组
                    switch(obj_responce.Ret)
                    {
                        case 0 : return '044000'; //转IVR成功
                        case 1 : return '044001'; //坐席错误
                        case 2 : return '044002'; //坐席不在通话中
                        case 3 : return '044003'; //坐席不在通话中
                        case 4 : return '044004'; //通话异常
                        case 5 : return '044005'; //通话类型错误
                        case 6 : return '044006'; //通话状态不对
                        case 7 : return '044007'; //通道错误
                        case 8 : return '044008'; //技能组错误
                        default: return '044999'; //其它转IVR错误
                    }
                    break;
				...
				switch(obj_responce.Code)
				{
					...
					return '024';
					break;
                    case ERROR_UNBUSY://强制置闲
                    unbusy();
                    return '025';
                    break;
                    case ERROR_BUSY://强制置忙
                    busy(1);
                    return '026';
                    break;
				}
	...
	$.wincall = function(settings) {
		var options = {
			...
			connect_type:'swf',//通道连接方式 swf：flash  amq：消息队列（网络条件不好的时候用）
			amq_uri_receive:'./mqclient/consumer.php?callback=?',
			amq_uri_send:'./mqclient/producer.php?callback=?'
		};
